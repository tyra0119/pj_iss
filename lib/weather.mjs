...
export function assessSite(hourly, peakDate, th = WEATHER_THRESHOLDS) {
  ...
  return {
    ...
    observable: cloudLowMid <= th.maxCloudLowMidPct && !(() => { const w = window(-1, 1); return !!w && (w.precip > 0 || w.precipProb >= th.obsMaxPrecipProbPct); })() && !(obs.vis != null && obs.vis < th.minVisibilityM) && !dangerous && !(((window(-1, 1) ?? {}).snow ?? 0) > 0),
    unstable: cloudLowMid <= th.maxCloudLowMidPct && !(((window(-1, 1) ?? {}).snow ?? 0) > 0) && (() => { const w = window(-1, 1); return !!w && (w.precip > 0 || w.precipProb >= th.obsMaxPrecipProbPct); })(),
    obsPrecipProb: (() => { const w = window(-1, 1); return w ? w.precipProb : null; })(),
    fog: obs.vis != null && obs.vis < th.minVisibilityM,                 // 霧・もや: 見えない
    snowNow: ((window(-1, 1) ?? {}).snow ?? 0) > 0,                      // 観測の前後 1 時間に雪: 雲があるので見えない
